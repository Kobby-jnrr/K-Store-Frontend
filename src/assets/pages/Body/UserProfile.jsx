...
const UserProfile = () => {
    ...
    const checkOrderStatusChanges = async () => {
      try {
        ...
        ordersData.forEach((order) => {
          ...
          const newStatus = order.items?.length
        ...
        const newStatusMap = {};
        ordersData.forEach(
          (o) => (newStatusMap[o._id] = computeOrderStatus(o))
        );
  ...
  const saveEdit = async () => {
    ...
    try {
      if (editingItem.type === "product") {
        for (let base of API_BASES) {
          try {
            await API.put(`${base}/products/${editingItem._id}`, formData, {
              headers: { Authorization: `Bearer ${token}` },
            });
            break;
          } catch {}
        ...
        setProducts((prev) =>
          prev.map((p) =>
            p._id === editingItem._id ? { ...p, ...formData } : p
          )
        );
      } else {
        for (let base of API_BASES) {
          try {
            await API.put(
              `${base}/auth/update/${user._id || user.id}`,
              formData,
              { headers: { Authorization: `Bearer ${token}` } }
            );
            break;
        ...
        const updatedUsername =
          formData.businessName?.trim() || `${user.firstName} ${user.lastName}`;
        setUser((prev) => ({
          ...prev,
          ...formData,
        ...
        sessionStorage.setItem(
          "user",
          JSON.stringify({ ...user, ...formData, username: updatedUsername })
        );
      }
      closeEditModal();
  ...
  const computeOrderStatus = (order) => {
    ...
    const allDeliveredOrRejected = order.items.every(
  ...
  const isVerified = isVendor
    ? verifiedStatus ?? user.verified ?? false
    : false;
  ...
  return (
    <div className="profile-page">
      ...
      <div
        className={`profile-grid ${isVendor ? "vendor-grid" : "customer-grid"}`}
      >
        {/* Profile Card */}
              ...
              <div className="vendor-product-grid">
                {products.map((p) => (
                  <div
                    ...
                    className="vendor-product-card hover-card"
                  >
                    <img
              ...
              {orders.map((order) => (
                ...
                >
                  <div className="order-header-new">
                    <span
                      className={`order-status ${computeOrderStatus(
                        order
                      ).toLowerCase()}`}
                    >
                    ...
                    <span className="order-total">
                      Total: GH₵{ceil2(order.total || 0).toFixed(2)}
                    </span>
                  </div>
      ...
      {editingItem && (
        <div className="edit-modal fade-in">
          <div className="edit-modal-content">
            <h3>
              {editingItem.type === "product" ? "Edit Product" : "Edit Info"}
            </h3>
      ...
      {activeOrder && (
        <div className="edit-modal fade-in">
          <div className="edit-modal-content">
            <h3>Order Details</h3>
            <p>
              <strong>Status:</strong> {computeOrderStatus(activeOrder)}
            </p>
            <p>
              <strong>Total:</strong> GH₵
              {ceil2(activeOrder.total || 0).toFixed(2)}
            </p>
            <p>
              <strong>Payment:</strong> {activeOrder.paymentMethod}
            </p>
            <p>
            ...
            <div className="order-items-new">
              {(activeOrder.items || []).map((item) => (
                <div
                  ...
                  className={`order-item-new ${
                    item.status === "rejected" ? "rejected-item" : ""
                  }`}
                >
                  <img
                  ...
                  <div className="item-details-new">
                    ...
                    {item.status === "rejected" ? (
                      <span className="rejected-msg">
                        ❌ Sorry, this item cannot be delivered
                      </span>
                    ) : (
